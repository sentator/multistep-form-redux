...
interface GeneralInformationProps {
	...
	cancelEditing: () => void;
	orderStatus: OrderProgressStatusItem;
}
...
const GeneralInformation: React.FC<GeneralInformationProps> = ({
	...
	const initialCountry = React.useRef(country);
	...
	const shouldDisableTrackingInput = orderStatus.label !== OrderProgressStatusLabel.PROCESSED;

	React.useEffect(() => {
		if (country && firstRender.current) {
			initialCountry.current = country;
			firstRender.current = false;
		}

		if (country && initialCountry.current && country.label !== initialCountry.current.label) {
			resetShopValue();
		}
	}, [country]);
		...
		<>
			...
			<div className="general-information-form__form">
				...
				<div className="general-information-form__row general-information-form__row--5">
					<SectionTracking name="trackNumber" id="input_track-number" readonly={shouldDisableTrackingInput} />
				</div>
				<div className="general-information-form__row general-information-form__row--6">
					...
					<div className="general-information-form__navigation">
						<NavigationLink title="Назад" to="/" onClick={cancelEditing} />
