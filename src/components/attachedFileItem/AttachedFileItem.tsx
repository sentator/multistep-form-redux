...
const AttachedFileItem: React.FC<AttachedFileItemProps> = ({ fileProperties, removeFile }) => {
	return (
		<span className="attached-file">
			<span className="attached-file__preview">
				{fileProperties.icon && (
					<span className="attached-file__icon">
						<img src={fileProperties.icon} alt={fileProperties.alt} />
					</span>
				)}
				{fileProperties.preview && (
					<img className="attached-file__img" src={fileProperties.preview} alt={fileProperties.alt} />
				)}
			</span>
			<button className="attached-file__btn-remove" type="button" onClick={removeFile}>
