...
// https://github.com/TanStack/table/issues/2064
// https://codesandbox.io/s/react-table-expanded-global-filter-1-kibuy?file=/src/App.js:711-726

export const ordersTableDefaultGlobalFilter = (rows, columnIds, filterValue, options) => {
	// Do not filter
	...
	return rows.filter((row) => {
		...
		const nestedExist =
			...
			(Array.isArray(row.subRows) &&
				!!ordersTableDefaultGlobalFilter(row.subRows, columnIds, filterValue).length);
